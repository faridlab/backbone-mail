//! Domain policy for MailTrackingValue aggregate
//!
...
/// Domain policy for MailTrackingValue — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailTrackingValue>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailTrackingValueDomainPolicy = PermitAllPolicy<MailTrackingValue>;
