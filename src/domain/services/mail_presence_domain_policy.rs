//! Domain policy for MailPresence aggregate
//!
...
/// Domain policy for MailPresence — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailPresence>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailPresenceDomainPolicy = PermitAllPolicy<MailPresence>;
