//! Domain policy for MailServer aggregate
//!
...
/// Domain policy for MailServer — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailServer>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailServerDomainPolicy = PermitAllPolicy<MailServer>;
