//! Domain policy for MailGatewayAllowed aggregate
//!
...
/// Domain policy for MailGatewayAllowed — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailGatewayAllowed>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailGatewayAllowedDomainPolicy = PermitAllPolicy<MailGatewayAllowed>;
