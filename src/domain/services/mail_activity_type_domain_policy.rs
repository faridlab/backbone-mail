//! Domain policy for MailActivityType aggregate
//!
...
/// Domain policy for MailActivityType — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailActivityType>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailActivityTypeDomainPolicy = PermitAllPolicy<MailActivityType>;
