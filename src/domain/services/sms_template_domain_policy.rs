//! Domain policy for SmsTemplate aggregate
//!
...
/// Domain policy for SmsTemplate — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<SmsTemplate>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type SmsTemplateDomainPolicy = PermitAllPolicy<SmsTemplate>;
