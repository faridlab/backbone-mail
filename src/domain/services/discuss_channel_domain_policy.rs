//! Domain policy for DiscussChannel aggregate
//!
...
/// Domain policy for DiscussChannel — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<DiscussChannel>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type DiscussChannelDomainPolicy = PermitAllPolicy<DiscussChannel>;
