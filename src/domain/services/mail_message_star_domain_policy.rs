//! Domain policy for MailMessageStar aggregate
//!
...
/// Domain policy for MailMessageStar — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailMessageStar>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailMessageStarDomainPolicy = PermitAllPolicy<MailMessageStar>;
