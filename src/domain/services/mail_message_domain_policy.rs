//! Domain policy for MailMessage aggregate
//!
...
/// Domain policy for MailMessage — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailMessage>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailMessageDomainPolicy = PermitAllPolicy<MailMessage>;
