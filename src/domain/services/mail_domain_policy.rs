//! Domain policy for Mail aggregate
//!
...
/// Domain policy for Mail — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Mail>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailDomainPolicy = PermitAllPolicy<Mail>;
