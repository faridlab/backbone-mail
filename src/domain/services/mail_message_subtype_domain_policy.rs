//! Domain policy for MailMessageSubtype aggregate
//!
...
/// Domain policy for MailMessageSubtype — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailMessageSubtype>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailMessageSubtypeDomainPolicy = PermitAllPolicy<MailMessageSubtype>;
