...
metaphor migration generate Invoice mail
metaphor migration run
...
(`mail` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed mail          # run Rust seeders
metaphor migration generate-seeds mail  # emit SQL seed files
```
