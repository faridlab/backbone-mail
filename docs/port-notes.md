...
| relay → backbone-notification transport wiring | ✅ closed | composed in the app under the **sentinel-company posture** (owner decision 2026-08-15): `gateway.platform_company_id` arms the composition — guarded routes behind an admin + `with_company_scope(platform_company)` gate, `dispatch_pending` sweeps the same fence, `QueueCommunicationPort` stages email dispatches as mail rows via `message_post` (`message_id` = the mail row id) and sms dispatches via `enqueue`. No RLS bypass anywhere; empty config = composition OFF (routes 404, job absent). Verified live 2026-08-15 on `backbone_mail_test`: seeded pending row → `dispatched=1` → row `sent` + exactly one `outgoing` mail, lifecycle events staged inside the fenced `notification.outbox_events`, gate 401/403/200, unarmed 404. |
